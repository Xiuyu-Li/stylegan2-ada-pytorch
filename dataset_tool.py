...
def open_artcifar10(tarball: str, *, max_images: Optional[int]):
    images = []
    labels = []

    with tarfile.open(tarball, 'r:gz') as tar:
        for batch in range(1, 6):
            member = tar.getmember(f'artcifar-10-batches-py/data_batch_{batch}')
            with tar.extractfile(member) as file:
                data = pickle.load(file, encoding='latin1')
            images.append(data['data'].reshape(-1, 3, 32, 32))
            labels.append(data['labels'])
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        ...
        elif os.path.basename(source) == 'artcifar-10-python.tar.gz':
            return open_artcifar10(source, max_images=max_images)
